...
class CiReleaseBundleError(RuntimeError):
    """Raised when the retained CI release bundle is incomplete or contaminated."""
...
def validate_ci_release_bundle(
    ...
    *,
    ...
    if missing:
        raise CiReleaseBundleError(f"CI release bundle is missing files: {missing}")
    if extra:
        raise CiReleaseBundleError(f"CI release bundle contains unexpected files: {extra}")

    for path in expected_paths:
        if path.stat().st_size <= 0:
            raise CiReleaseBundleError(f"CI release artifact is empty: {path.name}")
...
def main() -> int:
    ...
    print("PixelScope CI release bundle PASS")
    for path in paths:
        print(path.resolve())
    return 0
