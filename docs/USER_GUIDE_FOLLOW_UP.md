...
Status: **E8 implementation, owner Windows timing, both independent reviews, real E4 zero-impact smoke and exact implementation-HEAD Docs Windows/Ubuntu + E4 CI completed with no P1/P2 blocker; owner accepted PR #102 for integration.** [PR #102](https://github.com/delphykmc/pixelscope/pull/102) contains the code/policy changes; [Issue #100](https://github.com/delphykmc/pixelscope/issues/100) is the authoritative merge-SHA and measurement closeout record. The owner-local targeted suite is not represented as a fresh full-repository local pytest or installed Portable/Installer smoke.

The owner-local Windows screenshot-rendering test baseline is **18 passed in 138.72 s**. The module-scoped repository-copy fixture costs **58.85 s setup**; separate full integration calls are **20.51 s** (all PNGs absent), **19.98 s** (all present), **16.64 s** (corrupt PNG), and **16.38 s** (hook-only test that copies the repository). Issue [#100](https://github.com/delphykmc/pixelscope/issues/100) is authoritative for phase-level profiling, measured optimization, preserved 14-ID/strict-offline/source-link coverage, review and owner Windows acceptance. Close #100 with the reviewed and validated E8 merge SHA and measured closeout comment. Do not use `--skip-pytest` as a substitute for E8 test evidence.
...
- **E4:** Both same-repo base/head commits are still independently checked out at exact 40-character SHAs with read-only credentials. Python 3.10 runs the existing stdlib E3 selector **before installing the application, Pillow, pytest, Ruff or Qt**. The new fail-closed preflight allows no native GUI only with matching pinned SHAs, schema v1, an explicitly empty `selected_ids`, no warnings/manifest review/PNG change/removed-ID/profile-change flags, and an explicit no-selection reason. It writes `selection.json`, `report.json`, and `summary.md` to the original E4 artifact location. A selection error fails the job; **any selected ID, including removed/deferred IDs, retains the existing install, comparator regressions and genuine base/head GUI capture/review path**. Conservative E3 ownership/fallback and screenshot approval/provenance remain untouched; the broad E4 PR path trigger stays in place. The E8 PR itself modifies `ui-screenshot-diff.yml`, which E3 deliberately treats as screenshot-automation impact and may cause full GUI work **for this PR**. Unit regressions cover empty/pinned/mismatch/warnings/removed/deferred paths. The actual empty-path GitHub Actions run and pinned artifact are recorded below; a single observed workflow elapsed time is not a universal CI savings guarantee.
- **Docs validation / exact-tree policy (reviewer P2):** Preserve `push` **and** `pull_request` triggers, identical path filters, required PR Windows/Ubuntu checks and post-merge `main` push. A `pull_request` default checkout tests a **synthetic base+head merge tree**, whereas a `push` tests the **standalone head tree**: a matching `workflow_run.head_sha` alone is **not proof of equivalent validation**. The read-only non-`main` push guard now requires an actual PR workflow run for the same SHA/branch/open same-repo PR, acceptable queued/running (no conclusion) or completed success/failure status, the same recorded PR base SHA, the current synthetic merge commit with expected [base,head] parents, **and equality of its Git tree SHA with the standalone head commit's tree SHA**. Different/unknown trees, advanced base, force-push, unavailable or inconsistent run/merge/API state, PR path-filter omission and enqueue races run the full standalone push tests instead. This preserves merge-result validation **and** standalone head validation when their contents differ; PR checkout is not changed to `head`. `timed_out`, `stale`, `neutral`, cancelled/skipped/action-required and malformed run states do not justify skipping. The existing required PR check names `Validate on ubuntu-latest` / `Validate on windows-latest` are preserved; push contexts are renamed `Push validation on ...` so a green short-circuited push cannot impersonate the actual PR validation check. GitHub branch-protection details were not readable to this GitHub integration (403); no undocumented ruleset configuration is assumed. A qualifying push still consumes checkout/setup + a read-only lookup, not zero runner time. Branch-only and `main` push continue full coverage, and PR runs are never cancelled or skipped. Push/PR paths were aligned (including `test_user_guide_packaging.py`) and extended for E8 helpers/tests.
- **Real E4 zero-impact Actions acceptance:** a disposable, **closed unmerged** stacked [PR #103](https://github.com/delphykmc/pixelscope/pull/103) changed only non-rendered prose in `docs/user-guide/index.md` against pinned E8 base `0350c3b1426e570b7deccead246782a65efcb02f`, head `c57247c416137b1c682821eda4c9724267a6ef42`. [Windows E4 #36168428141](https://github.com/delphykmc/pixelscope/actions/runs/36168428141) **SUCCESS**; real E3 returned `selected_ids=[]` with `docs-prose-only`; real preflight emitted `no affected screenshots`; app/Pillow/Qt installation, comparator regression step and native capture all reported **SKIPPED**. Downloaded [artifact 10880055462](https://github.com/delphykmc/pixelscope/actions/runs/36168428141/artifacts/10880055462) and verified exactly `selection.json`, `report.json`, `summary.md` with matching exact SHA pair, zero screenshot/warning entries and report `status=no affected screenshots`. The temporary prose change was **not merged into E8 or main**. This measures the real skip route without extrapolating a universal runner-minute savings.
- **Policy evidence:** a valid same-HEAD PR must pass Git tree equality (not just matching workflow metadata SHA). [Push #36168374891](https://github.com/delphykmc/pixelscope/actions/runs/36168374891) reported `matching PR run AND identical merge/head Git tree` and its **Push validation on Windows/Ubuntu** contexts did not run heavy steps; separate [PR #36168380536](https://github.com/delphykmc/pixelscope/actions/runs/36168380536) uses original **Validate on Windows/Ubuntu** names and full checks. Unit negative cases keep both checks when synthetic merge/head trees differ, expected base changes, PR run terminates without validating, or API evidence cannot be established. GitHub branch protection API returned 403 for this integration, so no branch-protection configuration is asserted.
- **Unchanged:** Full Docs CI checks on the PR, strict network-blocked cold-cache MkDocs, E4 unknown-impact conservatism, E1 PoC, release validation, and manually dispatched User Guide publication/Pages approval. No opportunistic `--skip-pytest`, screenshot PNG/manifest byte change, or production UI edit. The final independent CI-policy review at [PR #102 HEAD `77fb842d...`](https://github.com/delphykmc/pixelscope/pull/102) found all three earlier P2 items resolved and no remaining P1/P2 blocker. [Docs PR #36168633375](https://github.com/delphykmc/pixelscope/actions/runs/36168633375) passed Windows and Ubuntu (214 docs-focused tests each, plus Windows 23 Help UI, 31 release-unit tests, Ruff/format 392 files, mypy 137 source files, network-blocked cold strict MkDocs/site); [E4 native #36168633332](https://github.com/delphykmc/pixelscope/actions/runs/36168633332) passed. The independent final review is recorded in PR #102; the linked Issue records the final merged result.
...
Historical/reproducible diagnostic command above remains available for subsequent targeted investigations; the E8 owner 3+1 post-A/B timing checkpoint is already complete, so no routine rerun is required for merge. Keep any future logs untracked under `temp/` and compare Windows local, Windows CI and Ubuntu CI separately. The suite contains 20 tests: original 18 plus two negative/cleanup regressions. **A/B itself did not change CI events or E4 selection**; those policies were subsequently reviewed and updated as the separate E8 CI-cost slice above, supported by real push/PR and native/zero-ID E4 Actions evidence.
