...
- `docs/user-guide/assets/screenshots/manifest.json` now records **seven actual checked-in images** with `legacy-unverified` provenance (never invent capture source SHA), **seven planned/missing scenario IDs** with page targets/technical follow-up, and three explicitly non-guide legacy diagnostic scenes. Two historical PNGs (Six-image and Floating Plots) remain unreferenced in topic Markdown until E5: `legacy-unreferenced` is a visible **migration debt**, not an approved placement. Five legacy topic images retain hard-coded Markdown references until E5. Only E1's Single View and RAW dialog are `isolated`; the other five remain `legacy-manual` until real scoped scene implementations pass their own Windows proof. Unimplemented gaps are `planned`, not alleged real UI captures.
