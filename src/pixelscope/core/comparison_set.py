...
class ComparisonSetSource:
    """One persistent native source reference and optional resolved RAW profile payload."""
...
@dataclass(frozen=True)
class ComparisonSet:
    """Qt-free v1 logical comparison state; Current Comparison Page remains derived."""
    ...
    def __post_init__(self) -> None:
        if self.kind != COMPARISON_SET_KIND:
            raise ComparisonSetError(f"unsupported comparison-set kind: {self.kind!r}")
        ...
        if active is not None and active.casefold() not in source_identities:
            raise ComparisonSetError("active source is not a member of the comparison set")
        if primary is not None and primary.casefold() not in source_identities:
            raise ComparisonSetError("primary source is not a member of the comparison set")
