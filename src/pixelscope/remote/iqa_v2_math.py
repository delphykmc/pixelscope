...
from pixelscope.remote.iqa_v2_domain import MeasurementSummary
...
def summary_from_grid(data: CompactAttributeData, value_kind: ValueKind) -> MeasurementSummary:
    ...
    if not np.any(mask):
        return MeasurementSummary.invalid()
    if np.any(~np.isfinite(weight[mask])) or np.any(~np.isfinite(weighted[mask])) or np.any(
        ~np.isfinite(squared[mask])
    ):
...
) -> dict[ComparisonMode, ScalarStatistic]:
    computed = compare_sources(spec, target, reference)
    if spec.value_kind is ValueKind.SIGNED:
        return {ComparisonMode.SIGNED_DELTA: computed["raw"]}
    return {
