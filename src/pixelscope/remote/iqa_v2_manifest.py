...
def parse_complete_manifest(root: Path, data: dict[str, Any]) -> ParsedManifestV2:
    ...
    for item in scenes_data:
        ...
        scene = _parse_scene(root, item, variants, attributes, all_source_ids)
...
def _parse_attribute(data: Any) -> AttributeSpec:
    ...
    if value_kind is ValueKind.POWER and operator is not ComparisonOperator.POWER_RATIO_A_OVER_B_DB:
        raise InvalidV2("power attribute requires power_ratio_a_over_b_db operator")
    if value_kind is ValueKind.SIGNED and operator is not ComparisonOperator.SIGNED_A_MINUS_B:
        raise InvalidV2("signed attribute requires signed_a_minus_b operator")
    return AttributeSpec(
        ...
        weighting_provenance=bounded_string(
            data, "weighting_provenance", V2_MAX_PROVENANCE_LENGTH
        ),
    )
...
def _parse_scene(
    ...
    if tuple(item.variant_id for item in sources) != expected_variants:
        raise InvalidV2(
            f"complete Scene {scene_id} source order/bindings must exactly match top-level variants"
        )
    ...
    if any(source_id in all_source_ids for source_id in local_ids):
        raise InvalidV2("source_id must be unique inside the result")
    ...
    if context_id != build_measurement_context_id(scene_id, sources, attributes, provenance):
...
def _parse_source_measurement(
    data: Any, attributes: tuple[AttributeSpec, ...]
) -> SourceMeasurementV2:
    if not isinstance(data, dict):
    ...
    source = Source(
        ...
        relative_path=bounded_string(data, "relative_path", V2_MAX_SOURCE_PATH_LENGTH),
        sha256=sha256,
    ...
    grids = {
        attribute.attribute_id: _parse_grid(
            grids_data[attribute.attribute_id], attribute.attribute_id, geometry
        )
        for attribute in attributes
    }
...
def _parse_context_provenance(data: Any) -> MeasurementContextProvenance:
    ...
    return MeasurementContextProvenance(
        representative_id=bounded_string(
            data, "representative_id", V2_MAX_PROVENANCE_LENGTH
        ),
        preprocessing_id=bounded_string(data, "preprocessing_id", V2_MAX_PROVENANCE_LENGTH),
        model_id=bounded_string(data, "model_id", V2_MAX_PROVENANCE_LENGTH),
        weighting_id=bounded_string(data, "weighting_id", V2_MAX_PROVENANCE_LENGTH),
        geometry_id=bounded_string(data, "geometry_id", V2_MAX_PROVENANCE_LENGTH),
    )
...
def _parse_geometry(data: Any) -> SceneGeometry:
    ...
    matrix = tuple(
        tuple(finite_float(value, "source_to_analysis") for value in row) for row in affine
    )
...
def _parse_grid(data: Any, attribute_id: str, geometry: SceneGeometry) -> GridGeometry:
    if not isinstance(data, dict):
...
def artifact_ref(data: dict[str, Any], key: str, limit: int) -> tuple[str, int]:
    item = data.get(key)
    if not isinstance(item, dict):
...
def bounded_string(data: dict[str, Any], key: str, maximum: int) -> str:
    value = data.get(key)
...
def bounded_list(data: dict[str, Any], key: str, maximum: int) -> list[Any]:
    value = data.get(key)
