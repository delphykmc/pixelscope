...
from pixelscope.remote.iqa_submission import IqaJobCreated, IqaJobRequest, IqaJobStatus, IqaResultReference, JobState
...
class HttpIqaJobClient(IqaJobClient):
    ...
    def __init__(self, base_url: str, *, timeout_seconds: float = 30.0, transport: httpx.BaseTransport | None = None) -> None:
    ...
    def get_status(self, job_id: str) -> IqaJobStatus:
        ...
        return IqaJobStatus(returned_id, _job_state(data.get("state")), completed, total, _optional_string(data, "message"))
    ...
    def cancel_job(self, job_id: str) -> IqaJobStatus:
        data = self._request_json("POST", f"/v1/iqa/jobs/{_job_id(job_id)}/cancel")
        returned_id = _required_string(data, "job_id")
        if returned_id != job_id:
    ...
    def _request_json(self, method: str, path: str, *, json: dict[str, object] | None = None) -> dict[str, Any]:
        try:
